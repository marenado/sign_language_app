...
import { Box, Button, Typography, TextField, Card, Modal, Fab } from '@mui/material';
...
const ModuleManagement = () => {
  ...
  const handleEditLesson = async () => {
    ...
    const currentModuleId =
      selectedModule?.module_id ??
      Object.keys(lessons).find((mid) => (lessons[mid] || []).some((l) => l.lesson_id === id));
  ...
  const fetchLanguages = useCallback(async () => {
    try {
      ...
      if (res.data.length > 0) {
        setSelectedLanguage(res.data[0].id);
      }
    } catch (error) {}
  }, []);
      ...
      >
          ...
          {languages.map((language) => (
            <Button
              ...
              sx={{
                padding: 0,
                border:
                  selectedLanguage === language.id ? '3px solid #5b21b6' : '3px solid transparent',
                borderRadius: '3px',
                boxShadow:
        ...
        >
          ...
          <TextField
            ...
            onChange={(e) => setModuleData({ ...moduleData, prerequisite_mod: e.target.value })}
            sx={{
              marginBottom: '15px',
              '& .MuiInputBase-root': {
                lineHeight: '1.5',
        ...
        <Box>
          <Typography
            variant="h6"
            sx={{ marginBottom: '20px', color: '#000000', fontWeight: 'bold' }}
          >
            Existing Modules for{' '}
            {languages.find((lang) => lang.id === selectedLanguage)?.name || 'Selected'} Sign
            Language
          </Typography>
          {modules.map((module) => (
            <Card
                ...
                {lessons[module.module_id]?.length > 0 ? (
                  lessons[module.module_id].map((lesson) => (
                    ...
                    >
                      ...
                      >
                        Difficulty: {lesson.difficulty || 'N/A'} | Duration:{' '}
              ...
              {videoSearchResults.map((video) => (
                <Box
                  ...
                  sx={{
                    ...
                    marginBottom: '5px',
                    cursor: 'pointer',
                    backgroundColor: selectedVideo?.video_id === video.video_id ? '#ddd' : '#fff',
                  }}
        ...
        <Dialog open={isDeleteDialogOpen} onClose={closeDeleteDialog}>
          ...
          <DialogActions>
            ...
            <Button onClick={handleDeleteModule} color="error" variant="contained" autoFocus>
              Delete
            </Button>
          </DialogActions>
        </Dialog>
